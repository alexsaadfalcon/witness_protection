...
from run_inference import getBoxes
...
def swapVideo(imagename='pratt.jpg', videoname='input1.mp4', outname='output.mp4'):
    ...
    out = cv2.VideoWriter(outname, cv2.VideoWriter_fourcc(*'MP4V'), 30.0, (frame_width, frame_height))
    ...
    while (cap.isOpened()):
        ...
        img2temp = np.copy(img2)
        img2temp2 = np.copy(img2)
        boxes = getBoxes(img2temp2)
        boxes[:,0] *= frame_height
        boxes[:, 2] *= frame_height
        boxes[:, 1] *= frame_width
        boxes[:, 3] *= frame_width
        # print(boxes)
        # input('TEST')
        if len(boxes) == 0 and boxesPrev != []:
            boxes = boxesPrev

        for box in boxes:
            try:
                rects2 = dlib.rectangle(int(box[1]), int(box[0]),
                                        int(box[3]), int(box[2]))
